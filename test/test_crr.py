...
def test_pull_from_insert(tmp_path: pathlib.Path) -> None:
    with sqlite3.connect(tmp_path / "a.db") as a, sqlite3.connect(
        tmp_path / "b.db"
    ) as b:
        exec(a, "CREATE TABLE X(rowid integer PRIMARY KEY);")
        ...
        assert fetch(a, "SELECT rowid FROM X") == [(1,)]
        assert fetch(a, "SELECT row_ts, row_peer FROM _synq_id") == [(1, 2)]
        assert fetch(a, "SELECT peer, ts FROM _synq_context") == [(1, 0), (2, 1)]
